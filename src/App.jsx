...
const ADMIN_NAME = "أ. محمد أبو سليمان";

// ============================================================
// ⭐ ProtectedRoute — حماية المسارات (تعمل بدون وميض)
// ============================================================
const ProtectedRoute = ({ children, allowedRoles }) => {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      ...
      // المستخدم مسجل لكن دوره غير مسموح
...
// ============================================================
// ⭐ SmartLoginRoute — يمنع ظهور Login إذا كانت الجلسة موجودة
// ============================================================
const SmartLoginRoute = () => {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      // ⭐ انتظار قصير حتى تُقرأ الجلسة من localStorage
      ...
      try {
        ...
        console.error("SmartLoginRoute error:", err);
        if (mounted) setStatus('show-login');
      }
    };
    ...
    return (
      <div className="status-page">
        ...
        <div className="status-card fade-in-up">
          ...
          <a href={buildWhatsAppLink()} target="_blank" rel="noopener noreferrer" className="whatsapp-btn">
...
const StatusStyles = () => (
  <style>{`
    ...
    * { box-sizing: border-box; }
    ...
    .blob-1 { width: 420px; height: 420px; top: -140px; right: -100px; background: radial-gradient(circle, rgba(96, 165, 250, 0.35), transparent 70%); }
    .blob-2 { width: 360px; height: 360px; bottom: -120px; left: -100px; background: radial-gradient(circle, rgba(147, 197, 253, 0.4), transparent 70%); }
    .blob-3 { width: 260px; height: 260px; top: 40%; left: 50%; transform: translate(-50%, -50%); background: radial-gradient(circle, rgba(191, 219, 254, 0.35), transparent 70%); }

    .status-card {
      position: relative; z-index: 2; width: 100%; max-width: 460px;
      background: #ffffff; border-radius: 28px; padding: 40px 32px 32px;
      box-shadow: 0 25px 60px rgba(30, 64, 175, 0.12), 0 8px 20px rgba(30, 64, 175, 0.06);
    ...
    .status-icon-wrapper.waiting { background: linear-gradient(135deg, #fef3c7, #fde68a); color: #d97706; box-shadow: 0 12px 24px rgba(217, 119, 6, 0.18); }
    .status-icon-wrapper.error { background: linear-gradient(135deg, #fee2e2, #fecaca); color: #dc2626; box-shadow: 0 12px 24px rgba(220, 38, 38, 0.18); }
    .status-icon-wrapper svg { width: 42px; height: 42px; position: relative; z-index: 2; }
    .status-icon-pulse { position: absolute; inset: -6px; border-radius: 32px; background: rgba(251, 191, 36, 0.35); animation: pulse 2s ease-in-out infinite; z-index: 1; }
    ...
    .status-greeting { font-size: 0.95rem; color: #64748b; margin: 0 0 8px; font-weight: 600; }
    .student-name { color: #1e40af; font-weight: 800; }

    .status-title { font-size: 1.5rem; font-weight: 900; color: #0f172a; margin: 0 0 14px; line-height: 1.35; letter-spacing: -0.4px; }
    .status-message { font-size: 0.95rem; color: #64748b; line-height: 1.8; margin: 0 0 24px; font-weight: 500; }

    .whatsapp-btn {
      display: flex; align-items: center; gap: 14px;
      background: linear-gradient(135deg, #25D366, #128C7E);
      border: none; border-radius: 18px; padding: 16px 20px;
      margin-bottom: 20px; text-decoration: none; color: #ffffff;
      text-align: right;
      box-shadow: 0 12px 28px rgba(37, 211, 102, 0.3), 0 4px 10px rgba(18, 140, 126, 0.2);
    ...
    .whatsapp-btn:hover { transform: translateY(-3px); box-shadow: 0 16px 36px rgba(37, 211, 102, 0.4), 0 6px 14px rgba(18, 140, 126, 0.3); }
    ...
    .whatsapp-icon-wrapper {
      ...
      display: flex; align-items: center; justify-content: center;
      flex-shrink: 0; color: #ffffff; position: relative; z-index: 2;
    }

    .whatsapp-content { display: flex; flex-direction: column; gap: 3px; flex: 1; position: relative; z-index: 2; }
    .whatsapp-title { font-size: 1rem; font-weight: 800; color: #ffffff; line-height: 1.3; }
    .whatsapp-sub { font-size: 0.78rem; font-weight: 600; color: rgba(255, 255, 255, 0.85); letter-spacing: 0.3px; }
    .whatsapp-arrow { color: rgba(255, 255, 255, 0.9); display: flex; align-items: center; transition: transform 0.25s ease; position: relative; z-index: 2; }
    .whatsapp-btn:hover .whatsapp-arrow { transform: translateX(-4px); }

    .divider { display: flex; align-items: center; gap: 12px; margin: 4px 0 20px; }
    .divider-line { flex: 1; height: 1px; background: linear-gradient(90deg, transparent, #e2e8f0, transparent); }
    .divider-text { font-size: 0.78rem; color: #94a3b8; font-weight: 700; letter-spacing: 1px; }

    .steps-list {
      display: flex; flex-direction: column; gap: 10px;
      background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px;
      padding: 16px 18px; margin-bottom: 22px; text-align: right;
    }
    .step-item { display: flex; align-items: center; gap: 12px; font-size: 0.88rem; color: #475569; font-weight: 600; line-height: 1.6; }
    .step-number {
    ...
    .btn-status.primary {
      ...
      color: #ffffff; box-shadow: 0 8px 18px rgba(37, 99, 235, 0.3);
    }
    .btn-status.primary:hover { transform: translateY(-2px); box-shadow: 0 12px 24px rgba(37, 99, 235, 0.4); }
    .btn-status.secondary { background: #ffffff; color: #475569; border: 1.5px solid #e2e8f0; }
    .btn-status.secondary:hover { background: #f8fafc; border-color: #cbd5e1; color: #1e293b; }
...
export default function App() {
  return (
    <>
      ...
      <BrowserRouter>
        <Routes>
          {/* ⭐ Login ذكي — يفحص الجلسة أولاً */}
          <Route path="/" element={<SmartLoginRoute />} />
